...
class Pan_Tompkins_Plus_Plus():
    ...
    def rpeak_detection(self, ecg, fs):
        ...
        if fs == 200:
            ...
            f2 = 18                                          # cutoff frequency to discard high frequency noise
        ...
        for i in range(LLp):
                ...
                if ((locs[i] - qrs_i[Beat_C-1]) >= round(1.66*test_m)) or ((locs[i] - qrs_i[Beat_C-1]) > round(1*fs)):     #it shows a QRS is missed
                    ...
                    if temp_vec.size:
                        ...
                        if pks_temp > THR_NOISE_TMP:  #If max peak in that range greater than Threshold3 mark that as a heart beat
                            ...
                            if i<(len(locs)-3):
                                temp_vec_tmp=ecg_h[int(qrs_i[Beat_C-3] + round(0.360*fs)-round(0.2*fs)+1):int(locs[i+3])+1]
            ...
            if pks[i] >= THR_SIG:
                ...
                if skip == 0:
                    ...
                    if y_i >= THR_SIG1:
                        ...
                        SIG_LEV1 = 0.25*y_i + 0.75*SIG_LEV1


                    SIG_LEV = 0.25*pks[i] + 0.75*SIG_LEV


            elif THR_NOISE <= pks[i] and pks[i] < THR_SIG:
                NOISE_LEV1 = 0.25 * y_i + 0.75 * NOISE_LEV1
                NOISE_LEV = 0.25*pks[i] + 0.75 * NOISE_LEV
            ...
            if NOISE_LEV != 0 or SIG_LEV != 0:
                THR_SIG = NOISE_LEV + 0.2 * (np.abs(SIG_LEV - NOISE_LEV))  #Calculate Threshold-1 for convoluted signal; above this R peak
                THR_NOISE = 0.3* THR_SIG                                   #Calculate Threshold-2 for convoluted signal; below this Noise
			
            ''' Adjust the threshold with SNR for bandpassed signal '''

            if NOISE_LEV1 != 0 or SIG_LEV1 != 0:
                THR_SIG1 = NOISE_LEV1 + 0.2*(np.abs(SIG_LEV1 - NOISE_LEV1)) #Calculate Threshold-1  for filtered signal; above this R peak
                THR_NOISE1 = 0.3* THR_SIG1                   #Calculate Threshold-2 for filtered signal; below this Noise
